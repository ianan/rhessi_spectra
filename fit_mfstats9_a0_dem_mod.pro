pro fit_mfstats9_a0_dem_mod
  ...
  for ii=3,nf-1 do begin
    ...
    o->set, fit_function='vth+thick2+drm_mod'
    ...
    det='d8'
    o->set, spex_specfile= 'mfstats9_fits/'+break_time(tr[0])+'_spec_'+det+'.fits'
    o->set, spex_drmfile= 'mfstats9_fits/'+break_time(tr[0])+'_srm_'+det+'.fits'
    o->set, spex_fit_time_interval=ftims
    o->set, fit_comp_param=[1e-3,1.5,1,1e-2,6,1000,20,15,1000,1,0,0.85]
    ...
    !p.multi=0
    !p.charsize=1.2
    figname='figs/mfl_'+break_time(ftims[0])+'_a0_dm_'+det+'.eps'
    set_plot,'ps'
    ...
    xyouts,xr[1]-2,10^(alog10(yr[1])-1.6), 'drm_mod[2]: '+string(p[11],format='(f6.2)'),align=1,color=0,/data,chars=1.2
