...
;    nstr = string(p[3], format = '(f5.2)') + 'x10!U35!N e!U-!Ns!U-1!N'
    gamstr = '!Mg!3: ' + string(p[6], format = '(f5.2)')+string(177b)+string(perr[6], format = '(f5.2)')
    ebstr = '!Me!3!DB!N: ' + string(p[5], format = '(f5.2)')+string(177b)+string(perr[5], format = '(f5.2)') + ' keV'
    ...
    !p.multi = 0
    !p.charsize = 1.2
    figname = 'mfl_' + break_time(ftims[0]) + '_bpow.eps'
    set_plot, 'ps'
...
;    oplot, erange[1] * [1, 1], [yr[0], 10 ^ (alog10(yr[1]) - 1.5)], lines = 1, thick = 2
    ...
    xyouts, xr[1] - 2, 10 ^ (alog10(yr[1]) - .8), gamstr + ', ' + ebstr, align = 1, color = 26, /data, chars = 1.2
