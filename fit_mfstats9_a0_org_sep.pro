pro fit_mfstats9_a0_org_sep
  ...
  ;
  ; Now fitting one flare with each detector separately
  ;
  ; 13-Nov-2023 IGH
  ; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  for ii = 0, nf - 1 do begin
    ...
    for di=0, nud-1 do begin
      ...
      o.set, spex_specfile = 'mfstats9_fits/' + break_time(resin[i].fpeak) + '_spec_org_'+dname+'.fits'
      o.set, spex_drmfile = 'mfstats9_fits/' + break_time(resin[i].fpeak) + '_srm_org_'+dname+'.fits'
      ...
      !p.multi = 0
      !p.charsize = 1.2
      figname = 'figs/mfl_' + break_time(ftims[0]) + '_a0_org_'+dname+'.eps'
      set_plot, 'ps'
      ...
      plot_oo, mide, dd.data, psym = 1, yrange = yr, ystyle = 17, xstyle = 17, xrange = xr, ytickf = 'exp1', $
        ...
        title = dname+': '+ftims[0] + ' to ' + anytim(ftims[1], /time, /trunc, /yoh)
