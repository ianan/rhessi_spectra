pro fit_mfl20061117_a0
  ...
  ;
  ; 29-Mar-2022 IGH
  ; 07-Nov-2023 Try fit with 1 keV to 100 keV binning instead
  ; ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  tmk2kev=0.086164
  ...
  o-> set, spex_specfile= 'fits/'+break_time(tr[0])+'_spec_sum_eb1.fits'
  o-> set, spex_drmfile= 'fits/'+break_time(tr[0])+'_srm_sum_eb1.fits'
  o->set, spex_fit_time_interval=ftims
;  o->set, fit_comp_param=[1e-3,1.5,    1,1e-2,6,   1000,20,15,1000]
  o->set, fit_comp_param=[4.74e-2,15.73*tmk2kev,1,8e-2,3.05,100,20,8.21,100]
  ...
  o->set, fit_comp_free = [1,0,0,1,0,0,0,0,0]
  o->dofit
...
;  o->dofit
  ...
  !p.multi=0
  !p.charsize=1.2
  figname='figs/mfl_'+break_time(ftims[0])+'_a0_eb1_100.eps'
  set_plot,'ps'
  ...
  xyouts,xr[1]-10,10^(alog10(yr[1])-.4),'No Fit: '+tmkstr+', '+emstr,align=1,color=25,/data,chars=1.2
  xyouts,xr[1]-10,10^(alog10(yr[1])-.8), nstr+', '+$
    delstr+', '+ecstr,align=1,color=26,/data,chars=1.2
  xyouts,xr[1]-10,10^(alog10(yr[1])-1.2), 'E!DUP!N: '+string(p[8],format='(i4)')+' keV',align=1,color=26,/data,chars=1.2
  xyouts,xr[1]-10,10^(alog10(yr[1])-1.6), '!Mc!3!U2!N: '+string(chisq,format='(f6.2)'),align=1,color=27,/data,chars=1.2
